      ...
      <Section>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div className="order-2 lg:order-1 flex justify-center relative">
            <div className="w-[280px] h-[560px] bg-background rounded-[3rem] border-[8px] border-surface-card relative overflow-hidden shadow-2xl">
              <div className="p-6 space-y-6">
                ...
                <div className="space-y-4">
                  <div className="bg-surface-card p-3 rounded-xl rounded-bl-none max-w-[85%] text-xs text-white/80">
                    Your Omronix verification code is: 884-219. Don't share it with anyone.
                  </div>
                  <div className="bg-primary/20 p-3 rounded-xl rounded-br-none max-w-[85%] ml-auto text-xs text-right text-primary">
                    Verified. Thank you.
                  </div>
                </div>
              </div>
            </div>
            <Card className="absolute -bottom-4 -left-4 p-4 border-l-4 border-l-secondary">
              <p className="text-xs font-heading font-bold text-white">98% Open Rate</p>
              <p className="text-[10px] text-white/40">Average SMS engagement</p>
            </Card>
          </div>
            ...
            <div className="space-y-4">
              {smsFeatures.map((feature, idx) => (
                <div key={idx} className="flex gap-6 p-6 hover:bg-white/5 rounded-2xl transition-all border border-transparent hover:border-white/5 group">
                  <span className="material-symbols-outlined text-secondary pt-1 group-hover:scale-110 transition-transform">{feature.icon}</span>
                  <div>
                    <h4 className="font-heading font-bold text-white text-lg mb-1">{feature.title}</h4>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div className="space-y-8">
            ...
            <div className="space-y-4">
              {[
                ...
                <Card key={idx} className="p-6 flex gap-6 items-center border-white/5">
          ...
          <div className="relative min-h-[400px] flex items-center justify-center">
            ...
            <img 
              ...
              alt="Voice Visual" 
              className="w-full h-auto rounded-3xl opacity-50 mix-blend-screen"
            />
            <Card className="absolute p-4 rounded-full flex items-center gap-4 border-secondary/30">
              <span className="material-symbols-outlined animate-pulse text-secondary">mic</span>
              <span className="font-heading font-bold text-sm tracking-tight">Connecting to node_04...</span>
            </Card>
          </div>
        </div>
      </Section>
      ...
      <Section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {[
            ...
            <div key={idx} className="text-center p-8 border-r border-white/5 last:border-0">
              <h3 className={`text-5xl font-heading font-bold mb-2 ${stat.color}`}>{stat.value}</h3>
              <p className="text-white/30 font-heading font-bold text-xs tracking-widest">{stat.label}</p>
            </div>
          ))}
        </div>
      </Section>
